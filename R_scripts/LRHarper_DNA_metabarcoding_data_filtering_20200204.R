...
DNA.true[163, "Assignment"] <- "Chironomus"
...
DNA.true[231, "Assignment"] <- "Parachironomus"
...
DNA.ass.bug.freq  <- DNA.ass.bug/c(DNA.ass.bug[272,])
...
DNA.ass.bug.freq <- tibble:::rownames_to_column(DNA.ass.bug.freq, "Assignment")

## Print contamination threshold based on max. level of assassin bug
## DNA contamination
max(DNA.ass.bug.freq[204,341])   # 0.0002361414
...
## all samples.
DNA.ass.bug.test <- DNA.ass.bug.test[-272,-341]
...
DNA.ass.bug.FP <- DNA.ass.bug.conversion*c(DNA.ass.bug.conversion[272,])

## Remove total row, reset row names, recreate Assignment column
DNA.ass.bug.FP <- DNA.ass.bug.FP[-272,]
DNA.ass.bug.FP <- tibble:::rownames_to_column(DNA.ass.bug.FP, "Assignment")

## Compare taxon richness of samples with and without threshold
## Convert sequence read counts to presence-absence
temp <- DNA.ass.bug[-272,]
...
## Make Assignment column row names
rownames(DNA.pos) <- DNA.pos$Assignment
DNA.pos <- DNA.pos[,-1]
...
## Create new dataframe containing the frequency of reads in each sample
DNA.pos.freq <- DNA.pos/c(DNA.pos[272,])
...
DNA.pos.freq <- tibble:::rownames_to_column(DNA.pos.freq, "Assignment")
...
max(DNA.pos.freq[-c(204,272),9])   # 0.0003758551
...
DNA.pos.FP <- DNA.pos.conversion*c(DNA.pos.conversion[272,])

## Remove total row, reset row names and recreate Assignment column
DNA.pos.FP <- DNA.pos.FP[-272,]
DNA.pos.FP <- tibble:::rownames_to_column(DNA.pos.FP, "Assignment")
...
## This would result in up to 77% taxa being removed.
## On average, 22% taxa are removed from samples.
...
## Make Assignment column row names
rownames(DNA.neg) <- DNA.neg$Assignment
DNA.neg <- DNA.neg[,-1]
...
## Create new dataframe containing the frequency of reads in each sample
DNA.neg.freq <- DNA.neg/c(DNA.neg[272,])
...
## Combine read frequencies with taxonomic assignment
DNA.neg.freq <- tibble:::rownames_to_column(DNA.neg.freq, "Assignment")
...
max(DNA.neg.freq[-c(204,272),38])   # 100%
...
DNA.SS.FP <- DNA.SS.conversion*c(DNA.SS.conversion[272,])

## Remove total row, reset row names and recreate Assignment column
DNA.SS.FP <- DNA.SS.FP[-272,]
DNA.SS.FP <- tibble:::rownames_to_column(DNA.SS.FP, "Assignment")
...
DNA.fam[154:156, "Assignment"] <- "Chironomidae"
DNA.fam[163, "Assignment"] <- "Chironomus"
DNA.fam[167, "Assignment"] <- "Cladopelma"
DNA.fam[169, "Assignment"] <- "Colymbetes fuscus"
DNA.fam[178, "Assignment"] <- "Dasybranchus"
DNA.fam[207, "Assignment"] <- "Hypoderaeum"
DNA.fam[214, "Assignment"] <- "Limnesia"
DNA.fam[216, "Assignment"] <- "Limnesiidae"
DNA.fam[218, "Assignment"] <- "Macropelopia"
DNA.fam[219, "Assignment"] <- "Macrothrix"
DNA.fam[231, "Assignment"] <- "Parachironomus"
DNA.fam[239:240, "Assignment"] <- "Podocopida"
DNA.fam[245, "Assignment"] <- "Polypedilum"
DNA.fam[248, "Assignment"] <- "Procladius"

## Also correct 'unknown' family assignment for Podocopida and Rotaria
DNA.fam[239:240, "Family"] <- "Podocopida"
DNA.fam[256:258, "Family"] <- "Philodinidae"

## Now merge read counts again by taxonomic assignment
## Anything with the same name in the data frame will be merged
DNA.fam <- ddply(DNA.fam, .(Assignment, Family), numcolwise(sum))

## Apply false positive threshold (0.0002361414%) to data
...
fam.freq[fam.freq <= 0.0002361414] <- 0
...
## Remove total row, reset row names, recreate Assignment column
fam.FP <- fam.FP[-272,]
fam.FP <- cbind(DNA.fam[,1:2], fam.FP)
...
DNA.fam.refine <- fam.FP[-c(17,22:23,41,48,70,74,115,122,125:127,143,160,167,
                            204,206,227:228,234,241,267),]
...
## Now create dataframe that only contains S/M/L size categories to be pooled 
## for each pond
DNA.fam.pool <- DNA.fam.refine[,which(grepl("Family|-S-|-M-|-L-", 
                                            colnames(DNA.fam.refine)))]
